...
export const withBaseMethods = <T extends BaseEntity>() =>
  signalStoreFeature(
    ...
    withMethods(
      (
        ...
        const actions = {
          ...
          filterChange: rxMethod<string>(
            pipe(
              tap((newFilter: string) => {
                patchState(store, { filter: newFilter });
                actions.searchEntities({ filter: newFilter });
              }),
            ),
          ),
    ...
    withHooks({
      onInit({ getFilters, searchEntities }) {
        getFilters();
        searchEntities({ size: 20, current: 1 });
      },
    }),
  );
